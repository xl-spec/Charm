class Collider {
    ...
    axeCollideSprite(axe, sprite) {
        ...
        for (let key in axe.hitbox_map) {
          ...
          if (dist(hitbox.x, hitbox.z, sprite.x, sprite.z) < (axeRadius + sprite.size / 2)) {
            return true;
          }
        }
        return false;
    ...
    projectileCollidePlayer(projectile, player) {
        ...
        const d = dist(projectile.position.x, projectile.position.z, player.x, player.z);
        const xzCollision = d < (projectileRadius + player.size / 2);
        
        if (!xzCollision){ // maybe improves performance? idk lol
            return false;   
        }
